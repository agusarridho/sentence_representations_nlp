...
object MainProb4_RNN_Dropout extends App {
  //-----------------------------------------------
  // Settings
  //-----------------------------------------------
...
//  val learningRateList = List(1e-3, 1e-4)
  ...
  var instanceModel = new RecurrentNeuralNetworkModelWithDropout(wordDim, hiddenDim,
    vectorRegularizationStrength, matrixRegularizationStrength, dropoutProb)
  var model: Model = instanceModel

  if (runGridSearch){
    gridSearch()
  }else{
    instanceModel.isTestTime = false
  ...
  def epochHook(iter: Int, accLoss: Double): Unit = {
    if (runGridSearch) {
      if (iter == numEpoch - 1) {
        instanceModel.isTestTime = true
        val trainAcc = 100 * Evaluator(model, trainSetName)
        val devAcc = 100 * Evaluator(model, validationSetName)
        val iterString = s"\rEpoch ${iter} | Word Size ${wordDim} | Hidden Size ${hiddenDim} | " +
      ...
      instanceModel.isTestTime = true
      println("\rEpoch %4d\tLoss %8.4f\tTrain Acc %4.2f\tDev Acc %4.2f".format(
        ...
        vecRegStrList.foreach(aVecRegStrength => {
          matRegStrList.foreach(aMatRegStrength => {
            learningRateList.foreach(aLearningRate => {
              ...
              model = new RecurrentNeuralNetworkModelWithDropout(
