...
object MainProb4_PreTrainedSumOfWord extends App {
  //-----------------------------------------------
  // Settings
  //-----------------------------------------------
...
//  var numEpoch = 50
  ...
  var wordDim = 40  //YOu can play with dim 10, 20, 30, 40, 50
  ...
  var model: Model = new PreTrainedSumOfWordVectorsModel(
    wordDim, vectorRegularizationStrength, s"trainedVectors_Dim_${wordDim}.txt")
      ...
      wordDimList.foreach(aDim => {
        vecRegStrList.foreach(aRegStrength => {
          learningRateList.foreach(aLearningRate => {
            ...
            model = new PreTrainedSumOfWordVectorsModel(
              wordDim, vectorRegularizationStrength, s"trainedVectors_Dim_${wordDim}.txt")
            StochasticGradientDescentLearner(model, trainSetName, numEpoch, learningRate, epochHook)
          })
        })
